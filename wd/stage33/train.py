...
def main():
    ...
    wandb_logger = WandbLogger(
        ...
        name="last_run_aide_all",                   # 可自定义每次训练的名字
        log_model=False                     # 自动记录模型权重
    )
    ...
    "inaturalist-2019-fgvc6",
        ...
        target_comps =NEWMEDAL + RDMEDAL + AIDEMEDAL

        merged_comps = list(target_comps)

        for c in comp_to_scen.keys():
            if c not in merged_comps:
                merged_comps.append(c)

        target_comps = merged_comps

        print("Total pairs loaded:", len(target_comps))
        filtered_pairs = [
            p for p in pairs
            if p.get("competition") in target_comps
        ]

        print(f"✅ Loaded {len(filtered_pairs)} pairs after filtering "
            f"from {len(pairs)} total samples.")
        print("Included competitions:", {p["competition"] for p in filtered_pairs})
        ...
        # print(f"✅ Loaded {len(val_filtered_pairs)} pairs after filtering "
        #     f"from {len(val_pairs)} total samples.")
        # print("Included competitions:", {p["competition"] for p in val_filtered_pairs})
        # val_pairs = val_filtered_pairs
        val_pairs = val_pairs
    ...
    n_val = len(val_pairs)
    n_take = int(0.01 * n_val)

    # 随机打乱索引
    indices = list(range(n_val))
    random.shuffle(indices)

    take_idx = set(indices[:n_take])
    ...
    for i, p in enumerate(val_pairs):
        if i in take_idx:
            val_to_pairs.append(p)
        else:
            new_val_pairs.append(p)

    # 更新
    pairs = new_val_pairs #val_to_pairs#pairs + val_to_pairs
    val_pairs = val_to_pairs
    ...
    ckpt_cb = ModelCheckpoint(
        ...
        filename="best-rm-{epoch:02d}-{val_acc_epoch:.4f}",
