# AutoRL-Bench

> 评测 Agent 自动进行 RL Post-training 的能力

---

## 1. 项目定位

**我们评测的是 Agent，不是模型。**

- 传统 Benchmark：评测模型能力
- AutoRL-Bench：评测 "让模型变好的 Agent" 的能力

---

## 2. 项目结构

```
AutoRL-Bench/
│
├── autorl_bench/           # Benchmark 核心
│   ├── server.py           # API 服务
│   ├── scenarios/          # 场景定义 (4个字段: model_path, data_path, baseline, metric)
│   └── evaluator.py        # 评测器 (封装 Inspect)
│
│
├── env/                    # 执行环境 (可 Docker 隔离)
│   ├── entry.py            # 统一入口
│   ├── training/
│   │   └── Dockerfile
│   └── eval/
│       └── Dockerfile
│
├── libs(in docker)/                   # RL 训练框架 (不上传 git)
│   ├── verl/               # git clone 预装
│   ├── trl/
│   └── ...                 # Agent 可用，也可自己拉 (不建议)
│
├── results/                # Benchmark 输出 (固定位置)
│   └── {scenario}_{timestamp}/
│       └── feedback.json   # {baseline, score}
│
├── log/                    # (ours) 服务日志
├── test/                   # (ours) 测试脚本
└── configs/                # 配置文件

------- Not part of the repo
├── assets/                 # 资源 (不上传 git)
│   ├── data/               # 数据集
│   └── models/             # 基础模型
│
├── workspace/              # 工作区 (不上传 git)
│   └── {scenario}_{timestamp}/
│       ├── code/           # 不限制 (config.yaml + 调用 libs/)
│       └── output/model/   # 必须有，评测用
├   ── agents/                 # Agent 实现 (ours + others)
│      ├── rdagent/            # (ours) 模仿 RD-Agent
│      │   └── ...
│      └── xxx_agent/          # (others) 其他人的 Agent
│          └── ...
```

---

## 3. 系统架构

```
┌─────────────────────────────────────────────────────────────────┐
│                    agents/                                       │
│  ┌─────────────┐  ┌─────────────┐                               │
│  │  rdagent    │  │ xxx_agent   │  ...                          │
│  │   (ours)    │  │  (others)   │                               │
│  └──────┬──────┘  └──────┬──────┘                               │
└─────────┼────────────────┼──────────────────────────────────────┘
          │                │
          └────────┬───────┘
                   │ 写入
                   ▼
┌─────────────────────────────────────────────────────────────────┐
│  workspace/{scenario}_{timestamp}/                               │
│  ├── code/           # 不限制                                    │
│  └── output/model/   # 必须有                                    │
└─────────────────────────────────────────────────────────────────┘
                   │ 读取 & 执行
                   ▼
┌─────────────────────────────────────────────────────────────────┐
│                    autorl_bench/                                 │
│  1. 执行训练代码                                                  │
│  2. 评测 output/model/                                           │
│  3. 输出 feedback                                                │
└─────────────────────────────────────────────────────────────────┘
                   │ 写入
                   ▼
┌─────────────────────────────────────────────────────────────────┐
│  results/{scenario}_{timestamp}/feedback.json                    │
│  { "baseline": 62.47, "score": 75.2 }                           │
└─────────────────────────────────────────────────────────────────┘
                   │ 读取
                   ▼
              Agent 获取反馈
```

**核心流程**:
```
Agent 写 workspace/  →  Benchmark 执行评测  →  输出 results/  →  Agent 读取
```

---

## 4. 接口设计

### 输入: workspace/

```
workspace/{scenario}_{timestamp}/
├── code/               # 不限制，Agent 随便写
└── output/model/       # 必须有，Benchmark 评测这里
```

### 输出: results/

```
results/{scenario}_{timestamp}/
└── feedback.json
```

```json
{ "baseline": 62.47, "score": 75.2 }
```

---

## 5. API

### GET /scenarios/{id}

返回 4 个字段：

```json
{
  "base_model_path": "assets/models/Qwen2.5-3B-Instruct",
  "train_data_path": "assets/data/gsm8k/train.jsonl",
  "baseline_score": 62.47,
  "metric": "accuracy"
}
```

| 字段 | 说明 |
|------|------|
| base_model_path | 基础模型位置 |
| train_data_path | 训练数据位置 |
| baseline_score | Baseline 分数 |
| metric | 评测指标 (accuracy / pass@1) |

### POST /evaluate

```json
// Request
{ "model_path": "workspace/xxx/output/model" }

// Response (调用 Inspect 评测)
{ "baseline": 62.47, "score": 75.2 }
```

---

## 6. agents/ (ours)

| Agent | 说明 |
|-------|------|
| rdagent | (ours) 模仿 RD-Agent |
| xxx_agent | (others) 其他人的 Agent |

---

## 7. 当前状态

| 项目 | 状态 |
|------|------|
| GSM8K Baseline | 62.47% |
| rdagent | 🚧 开发中 |

---

## 附录: Baseline

```
模型: Qwen2.5-3B-Instruct
数据: GSM8K test (1319 条)
准确率: 62.47% (824/1319)
```
